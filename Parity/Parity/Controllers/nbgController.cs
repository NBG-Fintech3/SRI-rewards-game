...
namespace Parity.Controllers
{
    ...
    public class NbgController : Controller
    {
        ...
        public string Get(int id)
        {
            ...
            {
                CreateUser(lines[4 * i], lines[4 * i + 1], lines[4 * i + 2]);
                response = responseContent;
            }
            int testScore = Int32.Parse(lines[3]);
            if (testScore > 51)
            {
                AddPoints(lines[3]);
                response = responseContent;
                openit("https://sjh.wtr.mybluehost.me/");                
            }
            else {
                openit("https://medium.com/parity-platform");                
            }
            return response;
        }
